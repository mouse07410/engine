...
if [ -d ${HOME}/openssl-4 ]; then
	chown -R ur20980 *
	ENG="" THREE="${THRE}" USE_DEVEL="True" OPENSSL_DIR=${O_DIR} ENGINESDIR=${E_DIR} ./gost-build-3.sh 2>&1 | tee ossl3m-build.txt
	chown -R ur20980 *
...
THRE="-3m-"
if [ -d ${O_DIR} ]; then
	THREE="${THRE}" ENG="on" OPENSSL_DIR=${O_DIR} OPENSSL_ROOT_DIR=${O_DIR} PROVDIR=${P_DIR} ENGINESDIR=${E_DIR} ./gost-build-3.sh 2>&1 | tee ossl3-build.txt
	if [ -r build/bin/gost.dylib ]; then
