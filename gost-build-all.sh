...
E_DIR="/opt/local/libexec/openssl3/lib/ossl-modules" 
THRE="-3m-"
THREE="${THRE}" OPENSSL_DIR=${O_DIR} ENGINESDIR=${E_DIR} ./gost-build-3.sh 2>&1 | tee ossl3-build.txt
if [ -r build/bin/gost.dylib ]; then
	sudo cp build/bin/gostprov.dylib ${E_DIR}/
	#sudo ln -sf ${E_DIR}/gostprov.dylib ${E_DIR}/gost.dylib
	sudo ln -sf ${E_DIR}/gostprov.dylib /opt/local/lib/ossl-modules/
